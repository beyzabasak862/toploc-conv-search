                                      ...
                                      int32_t* out_ids, float* out_scores)
{
    ...
    float ep_score = dot_f32_avx2_4acc(qp, idx.iq_vec(ep), idx.iq_dim);
    vis[ep] = gen;
    ...
    while (!cand.empty()) {
        ...
        for (size_t k = 0; k < nn && k < 8; ++k)
            __builtin_prefetch(idx.iq_vec(neigh[k]), 0, 1);
        ...
        for (size_t k = 0; k < nn; ++k) {
            ...
            if (vis[nid] == gen) continue;
            vis[nid] = gen;
            if (k + 8 < nn) __builtin_prefetch(idx.iq_vec(neigh[k + 8]), 0, 1);
            float sc = dot_f32_avx2_4acc(qp, idx.iq_vec(nid), idx.iq_dim);
            if (res.size() < ef) {
                cand.push_max(sc, nid);
                res.push_min(sc, nid);
                lower_bound = res.top_score();
...
class NativeQLR_V2 {
public:
    NativeQLR_V2(const std::string& export_dir)
      : idx_(export_dir)
    {
...
PYBIND11_MODULE(native_qlr_v2, m) {
    m.doc() = "native_qlr v2 — zero-alloc hot path, 4-acc AVX2 FMA, bounded top-K";
    py::class_<NativeQLR_V2>(m, "NativeQLR")
      .def(py::init<const std::string&>(), py::arg("export_dir"))
      .def("qlr", &NativeQLR_V2::qlr, py::arg("q"), py::arg("kp"), py::arg("kep"),
                                       py::arg("th"), py::arg("ef_default"), py::arg("ef_min"),
                                       py::arg("router_ef"), py::arg("s_max"), py::arg("topk"))
      .def("baseline", &NativeQLR_V2::baseline, py::arg("q"), py::arg("ef"), py::arg("topk"))
      .def("doc_ntotal", &NativeQLR_V2::doc_ntotal)
      .def("iq_ntotal",  &NativeQLR_V2::iq_ntotal)
      .def("dim",        &NativeQLR_V2::dim)
      .def("doc_entry_point", &NativeQLR_V2::doc_entry_point);
}

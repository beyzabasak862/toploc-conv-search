...
# Configuration
# -----------------------
INDEX_DIR      = "/home/toploc1/Datasets/toploc1/indexes/Dragon/HNSW"
QUERY_EMB_PATH = "/home/toploc1/Datasets/toploc1/indexes/Dragon_indexes/topics_dragon_embeddings.parquet"
...
def index_paths(m):
    return {
        "index": os.path.join(INDEX_DIR, f"treccast_hnsw_M{m}_dragon_mips.index"),
        "ids":   os.path.join(INDEX_DIR, f"treccast_hnsw_idsM{m}_dragon_mips.npy"),
    }


def output_paths(m):
    return {
        "summary": os.path.join(OUTPUT_DIR, f"dragon_toplocHNSW_M{m}_2019.parquet"),
        "metrics": os.path.join(OUTPUT_DIR, f"dragon_toplocHNSW_metrics_M{m}_2019.json"),
        "log":     os.path.join(OUTPUT_DIR, f"dragon_toplocHNSW_M{m}_2019.log"),
    }
...
q_emb = np.ascontiguousarray(
    np.hstack([q_emb_raw, np.zeros((len(q_emb_raw), 1), dtype=np.float32)]))
...
def run_for_m(m):
    ...
    try:
        log.info(f"##################  M = {m}  ##################")
        log.info(f"Loading index from {ip['index']} (~119 GB, be patient)")
        ...
        # ---- free the ~119 GB index before the next M loads ----
        index = None
        all_ids = None
        gc.collect()

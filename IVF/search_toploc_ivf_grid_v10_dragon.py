...
# ======================================================================
# v9 — UNIFIED plain-IVF + TopLoc-IVF grid, Guido-style measurement.
#
# Design decisions (and why):
#   * Baseline and TopLoc are measured in the SAME script, on the SAME
#     row sets, with the SAME timing protocol (warmup + median of N).
#     Every comparison is apples-to-apples.
#   * Latency is reported on three separate populations, never mixed:
#       - follow-up rows (429 queries)  -> the paper's actual claim
#       - first-turn rows (50 queries)  -> where TopLoc pays overhead
#       - whole-conversation break-even -> derived, amortization view
#     Both methods use identical denominators within each population.
#   * Recall (MRR@10 / NDCG@3 / NDCG@10) is computed ONCE per config at
#     K_METRICS: retrieval depth beyond rank 10 cannot change @10/@3
#     metrics, so sweeping k for recall would produce identical numbers.
#   * Latency IS swept over K_LATENCY_VALUES = [10, 100, 1000]: k sets
#     the heap cost of the posting-list scan, which is the term that
#     hides TopLoc's coarse-step saving. Speedup-vs-k is a result.
#   * Cache build happens ONCE per h, fully outside every timer, and is
#     reported separately (cache_build_ms_per_conv). It is charged only
#     where it honestly belongs: TopLoc's first-turn TOTAL cost and the
#     break-even computation.
#   * Baseline latency/recall per nprobe is computed once and reused
#     across all h values (it does not depend on h).
#   * Results are flushed to JSON after every (h, nprobe) cell.
# ======================================================================
...
METRICS_PATH = os.path.join(OUTPUT_DIR, "metrics_ivf_toploc_dragon_v10.json")
LOG_PATH     = os.path.join(OUTPUT_DIR, "search_ivf_toploc_dragon_v10.log")
...
# -----------------------
# Baseline search over an arbitrary row set
# -----------------------
...
# ======================================================================
# Pass 1: baseline, once per nprobe (independent of h). Reused later.
# ======================================================================
for nprobe in NPROBE_VALUES:
    ...
    # Latency on follow-up rows and first-turn rows, per k. Same protocol
    # and same row sets TopLoc will be timed on.
    lat = {}
    for k in K_LATENCY_VALUES:
        ...
        log.info(f"  k={k:>4}: followup={fu_ms:7.3f} ms/q  "
                 f"firstturn={ft_ms:7.3f} ms/q")
    ...
    for nprobe in NPROBE_VALUES:
        ...
        for k in K_LATENCY_VALUES:
            ...
            log.info(
                ...
                f"fu base={base_fu:7.3f} toploc={fu_t_ms:7.3f} "
                f"({speedup_fu:5.2f}x) | "
                f"ft total={ft_total_ms:7.3f} (ovh {ft_overhead:+6.3f}) | "
                f"breakeven={breakeven_turns:6.1f} turns | "
                f"conv speedup={conv_speedup:5.2f}x"
            )
